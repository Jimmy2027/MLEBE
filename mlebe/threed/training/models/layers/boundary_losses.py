"""
Copied from https://github.com/JunMa11/SegLoss/blob/master/losses_pytorch/boundary_loss.py
"""
import torch
...
def get_tp_fp_fn(net_output, gt, axes=None, mask=None, square=False):
    ...
    with torch.no_grad():
        ...
        if all([i == j for i, j in zip(net_output.shape, gt.shape)]):
            ...
            gt = gt.long()
            y_onehot = torch.zeros(shp_x).float()
            if net_output.device.type == "cuda":
                y_onehot = y_onehot.cuda(net_output.device.index).float()
...
class DC_and_BD_loss(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        bd_loss = self.bd(net_output, target, target)
