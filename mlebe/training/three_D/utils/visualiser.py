...
import numpy as np
import pandas as pd
from mlebe.training.three_D.utils import utils, html
from mlebe.training.three_D.utils.plot_logs import plot_logs
...
class Visualiser():
    ...
    def display_current_volumes(self, volumes, ids, split, epoch):
        ...
        for i in range(len(ids)):
            volume_stack = np.concatenate((volumes['input'][i], volumes['output'][i, 0:1], volumes['target'][i]),
                                          axis=0)
