...
data_dir = '/Volumes/something/mlebe_data'
data_sets = ['drlfom', 'mgtdbs', 'opfvta', 'ztau', 'hendrik_nvcz', 'irsabi']
template_dir = '/Volumes/something/mlebe_data/mouse-brain-atlases'
shape = (128, 128)
slice_view = 'coronal'
...
if not os.path.isfile('/Users/Hendrik/Documents/MLEBE/mlebe/Blacklist/blacklist.csv'):
    blacklist_df = pd.DataFrame(columns=['subject', 'session', 'acquisition', 'modality', 'slice', 'label'])
else:
...
for scan in range(len(img_data)):
    ...
    for slice in range(img_data[scan].shape[0]):
        temp_df = blacklist_df.loc[functools.reduce(operator.and_, (blacklist_df[item] == current for item, current in zip(['subject', 'session', 'acquisition', 'modality', 'slice'],[int(subject), session, acquisition, modality, slice]))), 'label']
        if temp_df.empty or temp_df.isna().all():
            ...
            plt.clf()
            plt.close()
            blacklist_df = blacklist_df.append(pd.DataFrame([[subject, session, acquisition, modality, slice, label]], columns=['subject', 'session', 'acquisition', 'modality', 'slice', 'label']))
