...
import mlebe.training.two_D.utils.general
from mlebe.training.two_D.network_trainer import network_trainer
    ...
    def train(self):
        ...
        if self.blacklist == True:
            if os.path.isdir(self.blacklist_dir):
                self.blacklist = mlebe.training.two_D.utils.general.write_blacklist(self.blacklist_dir)
            else:
