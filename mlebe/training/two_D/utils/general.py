...
def get_image_and_mask(image, mask, shape, save_dir, slice_view, visualisation=False, blacklist_bool=True, test=False):
    ...
    for i, m in zip(image, mask):
        ...
        img_temp, fitted_mask = remove_black_images(img_temp, fitted_mask, save_dir, test=test)
...
def remove_black_images(img, mask, save_dir=None, visualisation=False, test=False):
    new_img = img[:, :, :]
    new_mask = mask[:, :, :]
    if visualisation:
        before_img = img[:, :, :]
