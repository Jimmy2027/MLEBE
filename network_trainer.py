...
#todo combine binloss with dice
...
for loss in losses:
    ...
    for folder, data_gen_args in zip(folders, data_gen_argss):
        ...
        history = model.fit_generator(aug.flow(x_train, y_train, save_to_dir = save_dir + folder), steps_per_epoch=len(x_train) / 32, validation_data=(x_val, y_val), epochs=epochs, verbose=1, callbacks=[model_checkpoint, bidstest_callback])
